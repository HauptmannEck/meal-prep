          ...
          <div className="space-y-1">
            ...
            <ul className="space-y-3">
              {recipe.shoppingList?.map((item, idx) => (
                ...
                >
                  <div className={`mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${item.checked ? 'bg-teal-500 border-teal-500' : 'border-slate-600 group-hover:border-teal-500'}`}>
                    {item.checked && <svg className="w-3.5 h-3.5 text-slate-950" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                  </div>
                  <div className={`transition-all ${item.checked ? "opacity-40 line-through" : ""}`}>
